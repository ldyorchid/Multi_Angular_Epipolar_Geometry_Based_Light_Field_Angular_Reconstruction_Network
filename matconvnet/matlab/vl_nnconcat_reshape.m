...
if isempty(dzdy)
  inputs{2} = reshape(inputs{2}, [size(inputs{2}, 1),size(inputs{2}, 2),1,4]);
  y = cat(4, inputs{:});
  y = y(:,:,:,[61,1:6,62,7:54,63,55:60,64]);
  y = reshape(y, [size(inputs{2}, 1),size(inputs{2}, 2),8,8]);
  %disp(size(y));
else
  ...
  %end
  [h,w,~] = size(dzdy);
  dzdy = reshape(dzdy, [h,w,1,64]);
  dzdy = dzdy(:,:,:,[2:7,9:56,58:63,1,8,57,64]);
  opts.inputSizes{1} = [h,w,1,60];
  opts.inputSizes{2} = [h,w,1,4];
  start = 1 ;
  ...
  y{1} = reshape(y{1}, [h,w,1,60]);
  y{2} = reshape(y{2}, [h,w,4,1]);
end
