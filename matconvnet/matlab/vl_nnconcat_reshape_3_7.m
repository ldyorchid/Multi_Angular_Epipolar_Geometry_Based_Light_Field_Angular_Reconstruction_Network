function y = vl_nnconcat_reshape_3_7(inputs, dim, dzdy, varargin)
...
if nargin < 2, dim = 4; end;
if nargin < 3, dzdy = []; end;

if isempty(dzdy)
  inputs{2} = reshape(inputs{2}, [size(inputs{2}, 1),size(inputs{2}, 2),1,9]);
  y = cat(4, inputs{:});
  y = y(:,:,:,[41,1:2,42,3:4,43,5:18,44,19:20,45,21:22,46,23:36,47,37:38,48,39:40,49]);
  y = reshape(y, [size(inputs{2}, 1),size(inputs{2}, 2),7,7]);
  %disp(size(y));
else
  ...
  %end
  [h,w,~] = size(dzdy);
  dzdy = reshape(dzdy, [h,w,1,49]);
  dzdy = dzdy(:,:,:,[2:3,5:6,8:21,23:24,26:27,29:42,44:45,47:48,1,4,7,22,25,28,43,46,49]);
  opts.inputSizes{1} = [h,w,1,40];
  opts.inputSizes{2} = [h,w,1,9];
  start = 1 ;
  ...
  y{1} = reshape(y{1}, [h,w,1,40]);
  y{2} = reshape(y{2}, [h,w,9,1]);
end
