classdef Conv6D < dagnn.Filter4D
  ...
  methods
    ...
    function outputs = forward(obj, inputs, params)
      ...
      %TestInput = [64,64,2,2];
      %testFilter=[3,3,3,2,64];
      %testBais = [1, testFilter(end)];
      %TestInput = rand(TestInput, 'single');
      %testFilter = rand(testFilter, 'single');
      %testBais = rand(testBais, 'single');
      
      %pad    = [1,1,1,1,1,1]; % 3D higher/lower padding
      %stride = [1,1,1];         % 3D stride
      
      %outputs{1} = mex_conv3d(...
      %  TestInput, testFilter, testBais, ...
      %  'pad', pad, ...
      %  'stride', stride) ;
