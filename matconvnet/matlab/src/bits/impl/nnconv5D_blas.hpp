...
#ifndef __vl__nnconv5D_blas__
#define __vl__nnconv5D_blas__
...
namespace vl { namespace impl {
  ...
  nnconv5D_forward_blas(Context& context,
  ...
  template<vl::DeviceType deviceType, vl::DataType dataType> inline vl::ErrorCode
  nnconv5D_backward_blas(Context& context,
                       Tensor derData,
                       Tensor derFilters,
                       Tensor derBiases,
                       Tensor data,
...
template<vl::DeviceType deviceType, vl::DataType dataType> inline vl::ErrorCode
vl::impl::nnconv5D_forward_blas(Context& context,
                              Tensor output, double outputMult,
                              Tensor data, double dataMult,
                              Tensor filters,
                              Tensor biases,
...
inline vl::ErrorCode
vl::impl::nnconv5D_backward_blas(Context& context,
                               Tensor derData,
                               Tensor derFilters,
                               Tensor derBiases,
                               Tensor data,
                               ...
                               int padIn, int padOut)
{
  ...
  for (int image = 0 ; image < derOutput.getDimension(4) ; ++image) {
    ...
    ptrdiff_t derOutputOffset = (derOutput.getDimension(0) * derOutput.getDimension(1)
                                *derOutput.getDimension(2) * derOutput.getDimension(3)) * image ;
    ...
    if (derBiases) {
      ...
      (context,
       't',
       numOutputPixels, derOutput.getDimension(3),
       alpha, /* alpha */
    ...
    if (derData) {
      ...
      ptrdiff_t derDataOffset = (derData.getDimension(0) * derData.getDimension(1)
                                *derData.getDimension(2) * derData.getDimension(3)) * image ;
      for (int g = 0 ; g < numGroups ; ++ g) {
        ptrdiff_t filterGrpOffset = filtersVolume * numFiltersPerGroup * g ;
        ptrdiff_t tempGrpOffset = numOutputPixels * filtersVolume * g ;
    ...
    if (derFilters) {
      ...
      ptrdiff_t dataOffset = (data.getDimension(0) * data.getDimension(1)
                                *data.getDimension(2) * data.getDimension(3)) * image ;
      error = vl::impl::im2row5D<deviceType,type>::forward
      (context,
       (type*)tempMemory,
       (type*)data.getMemory() + dataOffset,
